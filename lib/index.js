...
module.exports = Adapter => class PostgreSQLAdapter extends Adapter {
    ...
    .then(results => {
      ...
      const addColumn = (type, field) => new Promise((resolve, reject) => {
        ...
        const dataType = typeKey in fieldDefinition ?
          postgresTypeMap.get(fieldDefinition[typeKey]) : primaryKeyType
  ...
  find (type, ids, options) {
    ...
    for (let field in options.match) {
      ...
      // Array containment.
      if (Array.isArray(value))
        where.push(`"${field}" @> array[${value.map(mapValue).join(', ')}]`)
      else {
        index++
        parameters.push(inputValue(value))
        where.push(`"${field}" @> array[$${index}]`)
      }
    }
  ...
  create (type, records) {
    ...
    const createRecords = `insert into ` +
      ...
      .join(', ') + `) values ` + records.map(record => {
        ...
        parameters.push(...orderedFields
          .map(field => inputValue(record[field])))
