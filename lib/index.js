...
export default Adapter => class PostgreSQLAdapter extends Adapter {
  ...
  connect () {
    ...
    new Promise((resolve, reject) => {
      ...
      const createTable = `create table if not exists "${mappedType}" ` +
        `("${keys.primary}" ${postgresTypeMap.get(primaryKeyType)} ` +
        `primary key)`
    ...
    .then(results => {
      ...
      const addColumn = (type, field) => new Promise((resolve, reject) => {
        ...
        const dataType = fieldDefinition[keys.type] ?
          postgresTypeMap.get(fieldDefinition[keys.type]) :
          primaryKeyType
  ...
  find (type, ids, options) {
    ...
    const { recordTypes, client, keys, options: { typeMap } } = this
  ...
  update (type, updates) {
    const { client, keys, options: { typeMap } } = this
  ...
  delete (type, ids) {
    ...
    const { keys, client, options: { typeMap } } = this
    let index = 0
