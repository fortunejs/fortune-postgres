...
export default Adapter => class PostgreSQLAdapter extends Adapter {
  ...
  connect () {
    ...
    if (!('typeMapping' in options))
      options.typeMapping = {}

    const { isNative, typeMapping, url } = options
    ...
    .map(type => new Promise((resolve, reject) => {
      ...
      const createTable = `create table if not exists ` +
        `"${type in typeMapping ? typeMapping[type] : type}" (` +
        ...
        ...Object.keys(recordTypes[type]).map(field => {
          ...
          return `"${field}" ${dataType}${isArray ? '[]' : ''}` +
            ...
            (' references "' +
            (link in typeMapping ? typeMapping[link] : link) +
            '" on delete set null') : ''}`
        }) ].join(', ') + `)`

      console.log(createTable) // eslint-disable-line
  ...
  find (type, ids, options = {}) {
    ...
    const { recordTypes, client, keys, options: { typeMapping } } = this
    ...
    if (ids) {
      where.push(`"${keys.primary}" in (` + ids.map(() => {
        index++
        return `$${index}`
      }).join(', ') + `)`)
      parameters.push(...ids)
  ...
  create (type, records) {
    ...
    const { recordTypes, keys, client,
      errors: { ConflictError }, options: { typeMapping } } = this
    ...
    return new Promise((resolve, reject) =>
      client.query(createRecords, parameters, error => {
        if (error) {
          ...
          if ((error.code || error.sqlState) === '23505')
  ...
  update (type, updates) {
    const { client, keys, options: { typeMapping } } = this
    ...
    return Promise.all(updates.map(update => new Promise((resolve, reject) => {
      ...
      const updateRecord = `update "` +
        (type in typeMapping ? typeMapping[type] : type) + `" ${set} ` +
      ...
      client.query(updateRecord, parameters, (error, result) => {
        if (error) {
          ...
          // http://www.postgresql.org/docs/8.2/static/errcodes-appendix.html
          if ((error.code || error.sqlState) === '42703') return resolve(0)
          return reject(error)
        }
  ...
  delete (type, ids) {
    ...
    const { keys, client, options: { typeMapping } } = this
    let index = 0

    const deleteRecords = `delete from ` +
      `"${type in typeMapping ? typeMapping[type] : type}"` + (ids ?
